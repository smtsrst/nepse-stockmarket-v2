...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Check cache
    if (losersCache && Date.now() - losersCache.timestamp < CACHE_DURATION) {
      return res.status(200).json(losersCache.data);
    }
    ...
    const losers = stocks
      ...
      .sort((a: any, b: any) => parseFloat(a.percent_change || 0) - parseFloat(b.percent_change || 0))
      .slice(0, 20)
      .map((s: any) => ({
        symbol: s.symbol || '',
        name: s.name || '',
    ...
    return res.status(200).json(losers.slice(0, limit));
  } catch (error) {
